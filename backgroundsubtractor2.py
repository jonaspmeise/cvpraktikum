...
class Tracking:
    ...
    def __init__(self, filePath, image_scaling, annotationFolder):
        #video stream
    ...
    def startTracking(self):
        ...
        while(1):
            ...
            if frame is not None:            
                ...
                boundingBoxes = self.getBoundingBox(self.getObjectMoments(fgMask, 5), 4, filter=True)
    ...
    @staticmethod
    def getObjectMoments(mask, dilation_iterations=5):
        currentCentroids = []
        
        #remove noise by dilating
        editedMask = cv2.dilate(mask,np.ones((3,3),np.uint8),iterations = dilation_iterations)
    ...
    def getBoundingBox(moments, max_boxes, filter=True):
        boundingBoxes = []
        for moment in moments:
            x,y,w,h = cv2.boundingRect(moment)
            boundingBoxes.append([x,y,w,h,w*h])  
        if filter:
...
program = Tracking('D:\cv_praktikum\inside\ch01_20200909115056_analysed_part_7.avi', 0.5,'D:\cv_praktikum\outside\ch04_20200908120907_analysed_part_18')
